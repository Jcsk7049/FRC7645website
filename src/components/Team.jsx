  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        ...
        console.warn("Team content fetch failed:", err);
        setHeroDesc(defaultContent.heroDesc);
        setParagraphs(defaultContent.paragraphs);
        setCoreValues(DEFAULT_CORE_VALUES);
      } finally {
    ...
    const fetchDivisions = async () => {
      try {
        ...
        console.warn("Divisions fetch failed:", err);
      } finally {
  ...
  return (
    <main className="container" style={{ paddingTop: "40px", paddingBottom: "80px" }} id="team-page">
      ...
      <section style={{ marginBottom: "64px" }}>
        <h2 style={{ marginBottom: "24px" }}>{t("team.valuesTitle")}</h2>
      ...
      {!divisionsLoading && divisions.length > 0 && (
        <section style={{ marginBottom: "64px" }}>
          <h2 style={{ marginBottom: "24px" }}>{t("team.divisionsTitle")}</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "24px" }}>
            {divisions.map(div => (
              ...
              >
                <div className="bento-card" style={{ overflow: "hidden", height: "100%", display: "flex", flexDirection: "column", transition: "box-shadow 0.2s" }}
                  ...
                  <div style={{ padding: "20px 24px", flex: 1, display: "flex", flexDirection: "column", gap: "8px" }}>
                    ...
                    {div.description && (
                      <p style={{ fontSize: "13px", color: "var(--text-muted)", lineHeight: "1.6",
                        ...
                        {stripMd(i18n.language === "en" ? (div.description_en || div.description) : div.description)}
                      </p>
                    )}
                    <span style={{ marginTop: "auto", paddingTop: "8px", fontSize: "13px", fontWeight: 700, color: "var(--accent)" }}>
                      {t("team.viewGroup")}
                    </span>
                  </div>
                </div>
      ...
      <section>
        <h2 style={{ marginBottom: "40px" }}>{t("team.milestonesTitle")}</h2>
